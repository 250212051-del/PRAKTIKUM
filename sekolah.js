...
formEkskul.addEventListener('submit', function(event) {
    ...
    // Tampilkan error jika tidak 10 digit TANTANGAN 1(disini saya tambahkan if dan else untuk benar dan salah nya)
    if (nisn.length !== 10) {
    errorMsg.textContent = "NISN tidak valid! Harus berjumlah 10 digit angka.";
    errorMsg.classList.replace('error-hidden','error-visible'); return;
    }else{
        errorMsg.classList.replace('error-visible','error-hidden'); errorMsg.textContent }
    ...
    //warna bordernya pakai if dan else if dan ini TANTANGAN 2
    if (ekskul === "Pramuka") {
    cardSiswa.style.border = "3px solid brown";
} else if (ekskul === "Palang Merah Remaja (PMR)") {
    cardSiswa.style.border = "3px solid red";
} else if (ekskul === "Klub Coding & Robotik") {
    cardSiswa.style.border = "3px solid blue";
} else if (ekskul === "Jurnalistik") {
    cardSiswa.style.border = "3px solid green";
}
...
// tombol hapus diproses melalui js bukan html TANTANGAN 3
